...
class SearchCollector(Collector):
    ...
    async def _brave(self, limit: int) -> list[str]:
        ...
        async with httpx.AsyncClient(timeout=20, headers={"X-Subscription-Token": key, "User-Agent": "OpenWeb-KR-Research/1.0"}) as client:
            for query in self.queries:
                try:
                    ...
                    found.extend(item["url"] for item in response.json().get("web", {}).get("results", []) if item.get("url"))
                except Exception: continue
                if len(found) >= limit: break
        return found[:limit]

    async def _google(self, limit: int) -> list[str]:
        ...
        async with httpx.AsyncClient(timeout=20, headers={"User-Agent": "OpenWeb-KR-Research/1.0"}) as client:
            for query in self.queries:
                try:
                    ...
                    response.raise_for_status()
                    found.extend(item["link"] for item in response.json().get("items", []) if item.get("link"))
                except Exception: continue
                if len(found) >= limit: break
        return found[:limit]
